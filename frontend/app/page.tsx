...
export default function Home() {
  ...
  const fetchData = async (symbol?: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/api/v1/history/${targetTicker}`);
      if (res.ok) {
        const data = await res.json();
        setHistory(data);
      } else if (res.status === 404) {
        // Auto-sync if data not found
        console.log(`No data found for ${targetTicker}, auto-syncing...`);
        setHistory([]);
        await handleSync(); // This will trigger sync and reload after 2s
      } else {
        setHistory([]);
      }
    } catch (e) {
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      await fetch(`http://localhost:8000/api/v1/sync/${ticker}`, { method: 'POST' });
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-6 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-lg text-sm font-medium hover:shadow-[0_0_20px_rgba(37,99,235,0.5)] transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
